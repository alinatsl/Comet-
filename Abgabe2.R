...
ggplot(data = filter(meine.daten_final, precipitation>0)) +
  ...
  xlab("Ausgeliehene Fahrraeder") +
  ylab("Temperatur(in Fahrenheit)") +
  ggtitle("Zusammenhang Fahrraeder und Temperatur bei Regen")+
  xlim(0,300)
...
Scatterplot <- plot_ly(data = meine.daten_final, x = meine.daten_final$mean_temperature, y = meine.daten_final$wind_speed, z = meine.daten_final$count, type = "scatter3d",
                       mode = "markers", marker = list(size = 5, opacity = 0.5), color = meine.daten_final$count,
                       text = paste("durchschnittliche Temperatur:", meine.daten_final$mean_temperature, 
                                    "<br>Windgeschwindigkeit:", meine.daten_final$wind_speed, "<br>Ausgeliehene Fahrraeder:", meine.daten_final$count), 
                       hoverinfo="text")
